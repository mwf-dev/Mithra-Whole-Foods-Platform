...
| GET | `/content-studio/products` | Link token | — | `{ products: [{ id, title, handle, thumbnail, image_count, product_status, origin, status, slide_count, filled_slide_count, archived, archived_at, archive_reason, updated_at, updated_by }], count }` — `status` is `not_started \| draft \| submitted \| approved` (the first is derived, never stored). `origin` is `catalog \| client`; client-origin rows are products the client proposed and have a synthetic `new_…` id. Archived rows are returned flagged, never filtered out |
| POST | `/content-studio/products` | Link token | `{ title, category?, pack_size?, price?, description?, ingredients?, notes?, images? }` | `201 { product_id }` — creates a **brief only**, never a catalog product |
| POST | `/content-studio/products/:productId/archive` | Link token | `{ archived: boolean, reason?, by? }` | `{ archived, archive_reason }` — the client's "remove from the shop" flag. Deletes nothing; creates a brief row to carry the flag if none exists |
| GET | `/content-studio/briefs/:productId` | Link token | — | `{ product: { id, title, handle, subtitle, description, thumbnail, images[] }, brief: { origin, status, summary, slides, proposal, archived, archive_reason, updated_at, updated_by, submitted_at, filled_slide_count } }` — for a `new_…` id the `product` is synthesised from `proposal` |
| PUT | `/content-studio/briefs/:productId` | Link token | `{ summary, slides, proposal, updated_by }` — whole-document autosave, sanitised server-side. `status` and the archive flag are **ignored** here; `proposal` only applies to a `new_…` product | Same shape as GET |
