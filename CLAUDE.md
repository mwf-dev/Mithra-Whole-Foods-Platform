...
| Infrastructure portal | **Implemented 2026-08-08.** Admin → **Infrastructure** (`src/admin/routes/platform/`). Monitors Vercel/Railway/Neon/Cloudinary/Stripe/SendGrid + this backend + the storefront: usage, run-rate cost projection, budget alerts (Slack/email), and a live "verify every endpoint" check covering config flags and store readiness. Read-only vendor tokens are entered in the UI (AES-256-GCM at rest) or fall back to env vars. Daily job at 06:00 UTC. → `docs/INTERNAL_DEVELOPER_PORTAL.md` |
| Content intake | **Implemented 2026-08-23**, extended 2026-08-25; inert until `CONTENT_STUDIO_TOKEN` is set. Link-only page at `GET /content-studio?t=` where the client writes per-product carousel copy, names their own slides, and uploads photos (via the existing file module → Cloudinary). They can also **propose new products** and **request removals** — neither touches the catalog: a proposal is a `product_brief` row with a synthetic `new_…` id, a removal is an `archived_at` flag. Review + paste-ready PIDS YAML in admin → **Content briefs**. → `docs/CONTENT_STUDIO.md` |
